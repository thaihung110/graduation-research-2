...
# Script to start/deploy the load-crypto-bronze SparkApplication
# This job reads crypto trades from Kafka and loads them into Iceberg bronze table

set -e

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
JOBS_DIR="$( cd "${SCRIPT_DIR}/../applications/spark/bronze-layer/jobs" && pwd )"
NAMESPACE="default"
JOB_NAME="load-crypto-bronze"
MANIFEST_FILE="${JOBS_DIR}/${JOB_NAME}.yaml"

echo "=========================================="
echo "Starting Load Crypto Bronze Spark Job"
echo "=========================================="
...
if kubectl get sparkapplication "${JOB_NAME}" -n "${NAMESPACE}" &>/dev/null; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "❌ Aborted. Use 'stop_load_crypto_bronze.sh' to stop the job first."
        exit 1
    fi
fi
...
echo ""
echo "   # Stop the job:"
echo "   ${SCRIPT_DIR}/stop_load_crypto_bronze.sh"
echo "=========================================="
