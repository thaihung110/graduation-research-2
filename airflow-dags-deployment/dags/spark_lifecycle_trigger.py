"""
spark_lifecycle_trigger.py

Trigger class phải nằm trong module riêng (KHÔNG được define trong DAG file).
Lý do: Airflow load DAG files với prefix hash 'unusual_prefix_...', khiến
Triggerer process không thể import lại class qua classpath thông thường.
"""
...
class SparkLifecycleTrigger(BaseTrigger):
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        try:
            async_config.load_incluster_config()  # synchronous - NO await
        except Exception:
            await async_config.load_kube_config()  # async - needs await
